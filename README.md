...
| `/checkout <index \| id-prefix \| name>` | Copy a **`cd … && claude --resume <id>`** command to the clipboard (paste into a new terminal to open that branch — works across projects, since in-session `/resume` only finds sessions of the current project). Also prints the in-session `/resume <id>` shortcut for same-project switches. |
| `/search <keyword> [more…] [10d\|3h\|2w] [all] [hidden]` | Full-text search across every conversation (titles + message text). All keywords must match (AND, case-insensitive); fork-inherited copies are skipped so parent content doesn't re-match in every fork; command-runner sessions are excluded unless `all`; hidden sessions are excluded unless `hidden` (a footer notes how many were skipped). Results are grouped by project and ranked by hits then recency; each shows a `«highlighted»` snippet that prefers your own words (command runs are listed without snippets). The `[n]` indices feed `/checkout` and `/hide` directly. |
| `/hide <index \| a-b \| id-prefix \| "title"> …` | Soft-hide one or more conversations (and their sub-branches) from `/tree`. Accepts index ranges (`1-10`) and title substrings (a quoted `"title"` hides **every** match). Reversible — the transcript is never deleted. Then re-renders the tree. |
| `/unhide <index \| a-b \| id-prefix \| "title" \| all> …` | Restore hidden conversations (cascades to sub-branches; same selectors as `/hide`). Then re-renders the tree. |
