...
| `/search <keyword> [more…] [10d\|3h\|2w] [all]` | Full-text search across every conversation (titles + message text). All keywords must match (AND, case-insensitive); fork-inherited copies are skipped so parent content doesn't re-match in every fork; command-runner sessions are excluded unless `all`. Results are grouped by project and ranked by hits then recency; each shows a `«highlighted»` snippet that prefers your own words (command runs are listed without snippets). The `[n]` indices feed `/checkout` and `/hide` directly. |
